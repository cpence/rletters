...
describe ApplicationController do
  ...
  describe '#render_localized_markdown' do
    context 'with a locale that exists' do
      ...
      it 'renders the file' do
        ...
        expect(controller).to receive(:render).with(file: path).and_return('')
  ...
  describe '#render_job_partial' do
    context 'with a partial that is present' do
      ...
      it 'succeeds' do
        expect(controller).to receive(:render).with(file: @path, locals: { klass: Jobs::Analysis::PlotDates }).and_return('')
