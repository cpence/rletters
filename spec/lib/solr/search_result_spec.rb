...
describe 'Solr::SearchResult' do
  ...
  describe '#documents' do
    ...
    context 'when no documents are returned' do
      it 'returns an empty array' do
        expect(Solr::Connection.search(q: 'uid:"fail"', def_type: 'lucene').documents).to be_empty
      end
    end
    ...
    context 'when Solr times out' do
      it 'raises an error' do
        ...
        expect { Solr::Connection.search(q: 'uid:"fail"', def_type: 'lucene') }.to raise_error(StandardError)
      end
    end
  end
  ...
  describe '#num_hits' do
    context 'when loading one document' do
      before(:each) do
        @result = Solr::Connection.search(q: 'uid:"doi:10.1111/j.1439-0310.2008.01576.x"', def_type: 'lucene')
      end
