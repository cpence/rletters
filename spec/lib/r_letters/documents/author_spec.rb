...
RSpec.describe RLetters::Documents::Author do
  describe '#full' do
    it 'raises nothing for some problematic values' do
      expect {
        described_class.new(full: 'for the Leishmaniasis East Africa Platform (LEAP) group')
      }.not_to raise_error
    end

    it 'returns whatever was passed into the constructor' do
      au = described_class.new(full: 'Greebleflotz, Johannes van der 123 Jr.')
  ...
  describe '#prefix' do
    it 'returns as expected with no comma' do
      au = described_class.new(full: 'Asdf van der Sdfg')
      expect(au.prefix).to eq('van der')
    end

    it 'returns as expected with comma' do
      au = described_class.new(full: 'Van der Sdfg, Asdf')
      expect(au.prefix).to eq('Van der')
    end
  end
  ...
  describe '#suffix' do
    ...
    it 'returns as expected with comma' do
      au = described_class.new(full: 'van der Sdfg, Jr., Asdf')
  ...
  describe '#to_lucene' do
    it 'creates simple query for last-only name' do
      expected = ['Last']
      actual = query_to_array(described_class.new(full: 'Last').to_lucene)
    ...
    it 'creates correct queries for F Last' do
      expected = ['F* Last']
      actual = query_to_array(described_class.new(full: 'F Last').to_lucene)
    ...
    it 'creates correct queries for FMM Last' do
      ...
      actual = query_to_array(described_class.new(full: 'FMM Last').to_lucene)
    ...
    it 'creates correct queries for First Last' do
      ...
      actual = query_to_array(described_class.new(full: 'First Last').to_lucene)
    ...
    it 'creates correct queries for First M M Last' do
      expected = ['F M* M* Last', 'First M* M* Last', 'First Last',
                  'F Last']
      actual = query_to_array(described_class.new(full: 'First M M Last').to_lucene)
    ...
    it 'creates correct queries for First MM Last' do
      expected = ['F M* M* Last', 'First M* M* Last', 'First Last',
                  'F Last']
      actual = query_to_array(described_class.new(full: 'First MM Last').to_lucene)
    ...
    it 'creates correct queries for First Middle Middle Last' do
      ...
      actual = query_to_array(described_class.new(full: 'First Middle Middle Last').to_lucene)
  ...
  describe '#citeproc' do
    it 'works as expected' do
      citeproc = described_class.new(full: 'First M M Last').citeproc
