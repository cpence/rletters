...
module RLetters
  module Analysis
    ...
    #
    # This is an interface implemented by all word frequency analyzer classes.
    # Currently we have two, one of which analyzes quickly by simply combining
    # the +tf+ values from the term vectors, and another which analyzes much
    # more slowly by reconstructing the full text from the +offsets+ variable.
    #
    ...
    #   @return [Integer] The number of tokens in the dataset.  If +ngrams+ is
    #     set, this is the number of ngrams.
    # @!attribute [r] num_dataset_types
    #   @return [Integer] The number of types in the dataset.  If +ngrams+ is
    #     set, this is the number of distinct ngrams.
    # @!attribute [r] df_in_corpus
    #   @return [Hash<String, Integer>] For each word (or ngram), the number of
    #     documents in the entire corpus in which that word appears
    class WordFrequency
      ...
      #
      # Before this function is called, the +@word_list+ variable should be set
      # with the full list of words available in the document.  It then
